...
//接收机
uint8_t RX_ADDRESS0[RX_ADR_WIDTH] = {0x20,0x22,0x2c,0x01,0x0C};	// 信道0 = TXDEV0_TX_ADDR = DEV1_RX_ADDRESS0{0x30, 0x37, 0x33, 0x34, 0x30}
uint8_t RX_ADDRESS1[RX_ADR_WIDTH] = {0x20,0x22,0x2c,0x01,0x3A};	// 信道1 = TXDEV1_TX_ADDR = DEV1_RX_ADDRESS0{0x31, 0x37, 0x33, 0x34, 0x30}
...
//		nRF24L01_Write_Buf(nRF_WRITE_REG | RX_ADDR_P1, RX_ADDRESS1, RX_ADR_WIDTH);	// 写接收端地址
	//nRF24L01_Write_Buf(nRF_WRITE_REG | TX_ADDR,    RX_ADDRESS0, RX_ADR_WIDTH);	//2-5通道使用1字节
		...
		nRF24L01_Write_Reg(nRF_WRITE_REG | RF_SETUP,    0x07);											//数据传输率：2Mbps，发射功率：0DB，
